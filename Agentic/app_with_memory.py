...
class Shoppingass:  
    ...
    def injection_node(self,state:State):
        ...
        if memory_json:
            try:
                ...
                existing_message.extend(mem["short_term_memory"])
                state.msg=existing_message
                return state
            except json.JSONDecodeError as e:
                return {"short_term_memory":["Error retrieving memory"]}
        print(f"No Redis cache found.")
        print("before inject:",state.msg)
        state.msg=existing_message
        print("after inject:",state.msg)
        print("exiting inject")
    ...
    def router_node(self, state: State) -> State:
        try:
            ...
            state.workflow = chain.invoke({'messages': state.msg}).tool_usage
            return state
    ...
    def message_to_memory(self,state:State):
        ...
        existing_memory_json=redis_client.get(redis_key)
        try:
        ...
        existing_memory["short_term_memory"].extend(msg)
